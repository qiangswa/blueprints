...
public class GraphMLWriter {
	...
	private IWriterEmitter writerListener;
    ...
    public void setExtendedWriter(IWriterEmitter writer){
    	this.setWriterListener(writer);
    }
    ...
    public void outputGraph(final OutputStream graphMLOutputStream) throws IOException {
        ...
        try {
    		...
    		if(getWriterListener()!=null){
    			getWriterListener().endGraphMLProperties(writer);
    		}
        	...
        	if(getWriterListener()!=null){
    			getWriterListener().endGraphML(writer);
    		}
	...
	public void writeDocument(XMLStreamWriter writer)
		...
		for (String key : keyset) {
			String type = vertexKeyTypes.get(key);
			if(isKnownType(type)){
		...
		if(getWriterListener()!=null){
			getWriterListener().endVertexKeyTypes(writer);
		}
		...
		if(getWriterListener()!=null){
			getWriterListener().endEdgeKeyTypes(writer);
		}
		...
		if(this.graphID==null){
			...
			writer.writeAttribute(GraphMLTokens.ID, this.graphID);
		}
		writer.writeAttribute(GraphMLTokens.EDGEDEFAULT, GraphMLTokens.DIRECTED);

		writeVertices(writer);

		writeEdges(writer,edgeKeyTypes);
		
		if(getWriterListener()!=null){
			getWriterListener().endGraph(writer,graph);
		}
		writer.writeEndElement(); // graph
	}

	protected void writeVertices(XMLStreamWriter writer)
			throws XMLStreamException {
		...
		for (Vertex vertex : vertices) {
		    ...
		    for (String key : keys) {
		        ...
		        if (null != value) {
		            ...
		            if(!isKnownType(type)){
		            	if(getWriterListener()!=null){
		            		getWriterListener().endVertex(writer,vertex,key, type);
		            	}
		            }else{
	...
	protected void writeEdges(XMLStreamWriter writer, Map<String, String> edgeKeyTypes) throws XMLStreamException {
		if (normalize) {
    ...
    protected boolean isKnownType(String type) {
		return GraphMLTokens.STRING.equals(type) || GraphMLTokens.INT.equals(type) || GraphMLTokens.LONG.equals(type) || GraphMLTokens.FLOAT.equals(type) || GraphMLTokens.DOUBLE.equals(type) || GraphMLTokens.BOOLEAN.equals(type);
	...
	public IWriterEmitter getWriterListener() {
		return writerListener;
	}

	public void setWriterListener(IWriterEmitter writerListener) {
